...
def test_udev_rule_file_path():
    ...
    txt = _script_text()
    assert (
        "/etc/udev/rules.d/99-storybot-spi.rules" in txt
...
def test_usermod_spi_present():
    ...
    txt = _script_text()
    assert (
        'usermod -aG spi "$INSTALL_USER"' in txt or "usermod -aG spi" in txt
...
def test_wrong_tool_not_used_for_spi1():
    ...
    txt = _script_text()
    assert (
        "config-by-hardware.py" not in txt
