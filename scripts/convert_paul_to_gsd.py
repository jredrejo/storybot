...
def extract_acceptance_criteria(text):
    ...
    return [
        (ac_id + ": " + title.strip(), desc.strip()) for ac_id, title, desc in matches
    ]
...
def generate_gsd_plan(paul_text):
    ...
    created_files, modified_files = (
        classify_files(files_modified_str)
        if isinstance(files_modified_str, str)
        else ([], [])
    )
    ...
    plan_title = "Phase {} Plan {}".format(
        phase_slug.replace("-", " ").title(), plan_num
    )
